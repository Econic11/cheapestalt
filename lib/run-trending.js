"use strict";
const amz = require("../api/amazon");
// lib/run-trending.js
// Automated pipeline: Google Trends → AI article generation → Supabase
// Runs every 6 hours via Vercel Cron
// Manual trigger: GET /api/run-all?secret=cheapestalt-stories-2026
//
...
async function buildArticlePage(article, slug, keyword, variationType, affiliateLink, style) {
  ...
  // Fetch real Amazon products to embed in article
  let realProductsHtml = "";
  try {
    const realProducts = await amz.searchProducts(keyword, 4);
    if (realProducts && realProducts.length > 0) {
      realProductsHtml =
        '<h2 style="font-size:20px;font-weight:800;margin:32px 0 16px;">Top Picks on Amazon</h2>' +
        amz.productGrid(realProducts, style);
      affiliateLink = realProducts[0].url;
    }
  } catch(e) {
    console.log("Amazon products skipped:", e.message);
  }
...
module.exports = async function handler(req, res) {
  ...
  // 1 keyword × 1 angle per run = 1 article, safe within 60s
