...
<header class="hdr">
  ...
  <div class="tag">Free · No signup needed</div>
</header>
<main class="wrap">
  <h1>${esc(title)}</h1>
  ...
  <div class="orig">
    <div class="orig-icon">${o.icon || "📦"}</div>
    <div class="orig-info">
...
function renderVariantHTML(data, slug, angle, qBase, pSlug) {
  ...
  const origCard =
    '<div class="orig-card">' +
    '<div class="orig-ico">' + esc(o.icon||"📦") + '</div>' +
  ...
  const altCards = alt.map((a, i) => {
    ...
    return '<div class="alt-card">' +
      '<div class="alt-top">' +
        '<div class="alt-ico">' + esc(a.icon||"🛒") + '</div>' +
        '<div style="flex:1">' +
