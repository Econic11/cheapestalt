...
// Fetches real products with direct links, prices, and images
  ...
  return new Promise(resolve => {
    ...
    const req = https.request({
      ...
      r.on("end", () => {
        ...
        resolve({ status: r.statusCode, data: p });
      });
    });
...
async function getToken() {
  ...
  if (!ID || !SECRET) throw new Error("Amazon credentials not configured");

  if (_tokenCache.token && _tokenCache.expiresAt > Date.now() + 60000) {
    return _tokenCache.token;
  }

  const r = await httpsPost("api.amazon.com", "/auth/o2/token",
    { "Content-Type": "application/json" },
    { grant_type: "client_credentials", client_id: ID, client_secret: SECRET, scope: "creatorsapi::default" }
  );

  if (r.status !== 200 || !r.data.access_token) {
    throw new Error("Token fetch failed: HTTP " + r.status);
  }
...
async function searchProducts(keyword, count) {
  ...
  const r = await httpsPost("creatorsapi.amazon", "/catalog/v1/searchItems",
    {
      "Content-Type":  "application/json",
  ...
  if (r.status !== 200) throw new Error("Search failed: HTTP " + r.status);
...
function productCard(p, style) {
  const primary = (style && style.primary) || "#2563EB";
  return '<div style="border:1.5px solid #E2E8F0;border-radius:12px;padding:16px;background:#fff;display:flex;flex-direction:column;gap:10px;">'
    ...
    + (p.price ? '<div style="font-size:20px;font-weight:800;color:' + primary + ';">' + p.price + '</div>' : '')
    + (p.rating ? '<div style="font-size:12px;color:#F59E0B;">★ ' + p.rating + (p.reviews ? ' (' + p.reviews + ' reviews)' : '') + '</div>' : '')
    + '<a href="' + p.url + '" target="_blank" rel="noopener" '
    + 'style="display:block;text-align:center;background:#FF9900;color:#0F172A;padding:10px;border-radius:8px;font-weight:700;font-size:14px;text-decoration:none;">'
    + '🛒 Check Price on Amazon</a>'
    + '</div>';
}

function productGrid(products, style) {
  if (!products || !products.length) return "";
  return '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:16px;margin:24px 0;">'
    + products.map(p => productCard(p, style)).join("")
    + '</div>';
}
